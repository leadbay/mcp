...
const ROUTING_EXAMPLES_OUT = join(
  REPO_ROOT,
  "packages",
  "core",
  "src",
  "routing-examples.generated.ts",
);
...
function runAssemble(): AssembleAndEmitOutput {
  ...
  const routingExamplesModule = emitRoutingExamples(result);
...
function cmdBuild(): void {
  ...
  const r5 = writeIfDifferent(ROUTING_EXAMPLES_OUT, routingExamplesModule);
  ...
  console.log(`[forge] ${ROUTING_EXAMPLES_OUT.replace(REPO_ROOT + "/", "")}: ${r5.changed ? "wrote" : "unchanged"}`);
...
function cmdCheck(): void {
  ...
  const d5 = diff(ROUTING_EXAMPLES_OUT, routingExamplesModule);
  ...
  if (
    ...
    if (!d5.matches) console.error(`[forge] ${ROUTING_EXAMPLES_OUT} is stale. Run pnpm prompts:build.`);
