...
| ≥ 1 lead returned (any batch)                              | "Enrich the top leads to reveal decision-maker email/phone"  | leadbay_enrich_titles({ dry_run: true }) — preview the volume + channels first; launch only after the user confirms (never a silent paid reveal) |
