...
| Batch is EMPTY and `computing_wishlist`/`computing_scores == true` (e.g. a just-created lens) | render the `next_steps` widget — it carries a single "Re-pull in ~30s" option. Do NOT report "no leads": the lens is warming up, not empty | leadbay_pull_leads (retry with same lensId after ~30s) |
