 ...
 *
 * When a tool is added to @leadbay/core or removed from openclaw.plugin.json,
 * these fail with a named-diff error. No magic tool counts.
 */
...
describe("contract: manifest ↔ code parity", () => {
  ...
  it("manifest has expected top-level shape", () => {
    ...
    expect(manifest.contracts.tools.length).toBe(11);
  });
});
