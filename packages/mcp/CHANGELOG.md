...
- **`leadbay_account_status` now stays fully SILENT on an unreadable quota, instead of reporting it** (product#3761 follow-up): 0.21.4 stopped the "reconnect" instruction but still had the agent say "quota is unreadable." Per product direction, a quota/plan read failure (the plan-less-org `quota_status` 401) is an internal diagnostic that is irrelevant to the user — so the guidance now tells the agent to omit quota entirely: do NOT mention quota, the error, or the 401, and (still) never tell the user to reconnect. The agent just answers the rest (user / org). Updated in both the `account-status.md.tmpl` rendering_hint and the `quota_error` output-schema description in `core/composite/account-status.ts`.
- **`leadbay_account_status` no longer volunteers the lens, and uses the lens NAME not the raw id when asked** (product#3761 follow-up): the agent was reporting the active lens unprompted and surfacing the bare numeric id (e.g. `40005`). The composite now resolves `last_requested_lens` → a new `last_requested_lens_name` field (best-effort via `/lenses`), and the guidance tells the agent not to volunteer the lens at all, and — only if the user explicitly asks which lens is active — to answer with the name, never the number. (Core 0.8.3.)

## 0.21.4 — 2026-06-18

- **`leadbay_account_status` no longer tells the user to reconnect when only the quota sub-call 401s** (product#3761 — the ACTUAL source of the reported hallucination): `account_status` fans out two calls — `/users/me` (identity) and `/organizations/{id}/quota_status` (quota). For an org with no billing plan (`plan: null`), the backend's `quota_status` returns **401** while `/users/me` succeeds with the same token. The composite captured that as `quota_error: {code: AUTH_EXPIRED, http_status: 401}` and embedded it in an otherwise-successful result — and BOTH the tool description's `rendering_hint` and the `quota_error` output-schema description instructed the agent: *"on 401/403 tell the user to reconnect / re-run OAuth."* So the agent told a perfectly-authenticated user to reconnect, every time, on a plan-less org. The guidance is now inverted: a 401/403 on `quota_error` is explicitly NOT a broken login (the `user`/`organization` fields in the same response were fetched with the same token and succeeded — commonly it's just an org with no plan), the agent must NOT tell the user to reconnect, and should only report that quota is unreadable. Fixed in both the promptforge template (`account-status.md.tmpl` rendering_hint) and the hand-authored `quota_error` output-schema description in `core/composite/account-status.ts`. (Core 0.8.2.)

## 0.21.3 — 2026-06-18

- **Local (stdio) MCP: the agent no longer narrates a one-off 401 as "reconnect Leadbay"** (product#3761, companion to the 0.21.2 hosted-path fix): the 401 "hallucination" also reaches users on the npx-installed stdio server. Leadbay bearer tokens don't expire on a timer, so the client treats a 401 as a transient server-side blip and auto-retries the GET once; only a *persistent* 401 surfaces, as an `AUTH_EXPIRED` envelope whose message already avoids blaming the user's login. But the agent still occasionally read that lone `AUTH_EXPIRED` / 401 as an auth failure and told the user to log in / reconnect the connector — even though the very next call worked. A new always-on server-instruction paragraph (`TRANSIENT_401`, from `snippets/server-instructions/transient-401.md`) tells the agent that a single 401 is almost always a brief Leadbay-side hiccup the client already retried, to silently retry once more and carry on, and to never turn a one-off 401 into a "your connection needs re-authenticating" message — surfacing it only if calls keep failing, framed as a possible Leadbay-side issue or logout and offered to `leadbay_report_friction`. No change to the retry count or the `AUTH_EXPIRED` code (both locked by existing tests); a genuinely persistent 401 still surfaces, so a real logout/revocation is still reported.

## 0.21.2 — 2026-06-18

- **Hosted MCP no longer "hallucinates" a 401 on a connect that actually works** (product#3761): the Fly endpoint answers an unauthenticated / expired `POST /mcp` with `401` + `WWW-Authenticate` — the RFC 9728 / MCP-authorization OAuth challenge that makes the host run sign-in / silent token-refresh (added in 0.21.0). That handshake is correct and unchanged. The bug was the challenge's *JSON body*, which carried human-readable prose ("Authentication required. Sign in with Leadbay." / "Access token is invalid or expired. Sign in with Leadbay again."). A spec-compliant client drives the whole OAuth flow from the status + header and never reads the body, but Claude's host surfaces the body to the LLM — which then told the user to go reconnect the connector even though the host had already refreshed the token and the immediate retry succeeded. The 401 challenge body is now empty; the expired-vs-missing signal still rides in the `WWW-Authenticate` header (`error="invalid_token"` for expired), so no protocol signal is lost. No body → nothing for the agent to parrot as a spurious re-auth instruction.
