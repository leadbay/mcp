...
/users/telemetry`, read from `GET /users/me`), so it works the same on the
hosted/web connector as on a local install. On the hosted server a user who
opts out has their telemetry suppressed per-request. Requires the backend
`telemetry_enabled` field (leadbay/backend) + api-specs contract.
