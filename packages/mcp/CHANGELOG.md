...
- **OAuth-on-install for the Claude Desktop `.dxt` now opens the browser reliably**: the bundled stdio server runs the loopback OAuth flow at startup and launched the browser via `spawn("open" | "xdg-open" | "cmd", …)` — bare command names resolved through `PATH`. Claude Desktop spawns `.dxt`/`.mcpb` extensions with a **sanitized environment whose `PATH` doesn't contain those launchers**, so the spawn failed with `ENOENT`, no browser ever opened, and the only diagnostic went to MCP stderr (invisible to the user) while the server dangled on the 5-minute callback wait. `openInBrowser` now launches via the OS launcher's **absolute path** (`/usr/bin/open`, `%SystemRoot%\System32\cmd.exe`, `/usr/bin/xdg-open`) with the bare-command `PATH` lookup kept only as a fallback — which is what restores the click-Install → browser-opens UX on every desktop OS. If the launcher genuinely can't be found, bootstrap no longer dangles for the full callback timeout: it aborts fast and the first tool call returns an `AUTH_MISSING` envelope (the channel the user actually sees) telling them to restart the extension to retry sign-in.
