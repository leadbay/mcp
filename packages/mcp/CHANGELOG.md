...
- **Installer pins the region the token ACTUALLY belongs to, not just the GeoIP guess** (product#3761, the deepest root cause of the "401 every startup"): the GUI installer (`npx … installer`) detected the region via stargate, which is a GeoIP lookup on the user's *current IP* — not which backend owns their account. A wrong guess (travel, VPN, a FR-account holder on a US IP, an unmapped country) wrote the wrong `LEADBAY_REGION` into the MCP client config, so every request then 401'd against the wrong backend on every startup. The installer now VERIFIES the freshly-minted token against `/users/me` (new `verifyTokenRegion` in `oauth.ts`): it probes the GeoIP-preferred region first, falls back to the other, and pins whichever the token authenticates against — surfacing "corrected from X" in the account label when the guess was wrong. If neither backend authenticates (a genuinely bad token, or a transient blip), it falls back to the detected region and lets the MCP's own startup auto-probe be the final safety net. This removes the deterministic wrong-region 401 at its source, rather than just declining to narrate it.
