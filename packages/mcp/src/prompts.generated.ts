...
- \`leadbay_top_accounts_to_activate\` (optional args: count, erp_extract, benchmark, territory): Build a ranked account-activation plan — the accounts with the most money still on the table, each with a motif, a pitch and a checklist. Ranks by cash-to-capture with the client's revenue extract; delivers an honest Leadbay-only conquest plan without it. Every figure carries its source. Uses \`leadbay_bulk_qualify_leads\`, \`leadbay_enrich_titles\`, \`leadbay_import_leads\`. Trigger on "top 50 accounts to activate", "where's the cash in my base".
- \`leadbay_work_campaign\` (optional args: campaign, mode): Work a campaign as a real outreach session: pick the campaign, assess what the user has (phones / emails / coords), then PROPOSE the right session mode (call sheet, email sheet, enrich titles first, map). After they pick, render — and as they dictate outcomes per lead, record both note + epilogue via \`leadbay_report_outreach\` in one round trip.`;
