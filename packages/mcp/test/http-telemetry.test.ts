...
describe("hosted HTTP MCP — telemetry wiring (product#3876)", () => {
  ...
  it("the wrapper injects identity into friction captures too (not just tool calls)", () => {
    // These fire per HTTP request when leadbay_report_friction runs. Before the
    // fix they fell through to the base handle with NO identity — and since the
    // HTTP base never identifies, they buffered until shutdown and flushed
    // anonymous. The wrapper must forward identity for them.
