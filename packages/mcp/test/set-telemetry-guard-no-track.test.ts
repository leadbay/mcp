/**
 * Codex P2 (product#3879): leadbay_set_telemetry is granular, but opt-out
 * attempts must still avoid recording the user's opt-out intent.
 *
 * Unlike composite tools, this granular privacy control does not require
 * `_triggered_by`. When the model does include `_triggered_by`, the server's
 * set-telemetry privacy exceptions suppress/sanitize the paths that would
 * otherwise record the opt-out prompt.
 */
...
describe("leadbay_set_telemetry — granular privacy-control telemetry guards (Codex P2)", () => {
  it("missing _triggered_by is allowed for the granular opt-out and still emits NO analytics pair", async () => {
    mockHttp([
      {
        method: "GET",
        path: "/1.6/users/me",
    ...
    // The load-bearing guarantee: the opt-out attempt is NOT tracked.
