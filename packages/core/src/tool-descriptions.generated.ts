...
(default 45); a longer job returns \`still_running\` + \`next_poll\` — hand off
to \`leadbay_lead_job_status\`. Jobs run ≤30 min, results kept 30 days.
...
phone 250c, success-only). The gate is enforced in code: a paid call
(\`qualify: true\` and/or \`channels\`) is WITHHELD unless it carries
...
**Exclusions ("no chains") are enforced by QUALIFICATION, not the free
match** — \`filters\` has no exclusion key. Put the positive inverse in the
seed, then \`qualify: true\`: IBP anti-patterns score violators negative and
\`min_ai_score: 0\` drops them. Durable exclusion → \`leadbay_refine_prompt\`.
In a FREE preview, drop violating rows and say the tier doesn't enforce.
