...
**Two ways out of an exhausted window.** Wait for the window reset (\`resets_at\` in each quota entry), or top up AI credits — a top-up is not bound to the window, so it clears the throttle immediately. When you report a 429 / quota exhaustion, state both and let the user pick which one they want.

**The top-up link comes from \`leadbay_create_topup_link\`.** When the user asks for it, call that tool and surface the returned Stripe checkout URL as a clickable link. The user completes payment in their browser; nothing is charged by generating the URL. For subscription changes (plan upgrade / payment method), use \`leadbay_open_billing_portal\` instead.

**After a user tops up, do NOT keep refusing — RETRY.** If the user signals they topped up / bought credits / added credits, the previous QUOTA_EXCEEDED is invalidated the moment the Stripe webhook lands. RE-CALL \`leadbay_account_status\` to pick up the new state AND retry the originally failed call. The retry itself does not require a successful account_status check first — a topped-up user has cleared the throttle whether or not your cached snapshot reflects it yet. If the retry hits the wall again, state the reset time and the top-up route again. **A stale quota snapshot is never a reason to gate-keep a topped-up user.**
