...
**Two-pass spending pattern — the default for channel purchases.** A single
qualify+channels pass buys emails even for leads the SAME job disqualifies
(measured on staging: a third of enrichment spend landed on ai-negative
leads). Instead: pass 1 with \`qualify: true\` and NO channels → read the
verdicts; pass 2 with \`channels\` and \`lead_refs\` restricted to the
positively-qualified leads (their \`lead_id\`s, or \`contact_id\` to pin the
matched person). The cache makes pass 2 cost only the channels. Collapse to
one pass only when the user explicitly wants channels regardless of verdict.
