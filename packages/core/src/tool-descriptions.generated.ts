...
| 3 | Enrich top leads | \`leadbay_enrich_titles\` — free preview, then a consented paid reveal |
| 4 | Add these to my CRM | *nothing* — **your own** CRM connector |
| 5 | Run this every morning | *nothing* — hands off to the host's scheduler |

Steps 1 and 2 carry \`branches[]\`, step 3 carries \`spend\` + \`quota_note\`, and steps 4–5 carry \`handoff\` prose instead of \`args\`. Every step also carries \`explain\` (say this BEFORE firing) and \`next_steps\` (\`{question, options[]}\` — already the widget's shape, map it verbatim).
...
**Step 3 runs in two beats — free first, paid only on consent.** Beat 1 omits \`titles\` and returns \`mode:"discover"\`, the free preview of available job titles; say plainly that nothing has been spent. Beat 2 asks the user to pick 2–3 leads to really enrich, states the cost BEFORE they choose, and only then calls again with the chosen \`titles\` + \`confirm:true\` + \`email:true\` — a real paid reveal, polled via \`leadbay_bulk_enrich_status\` until done, reporting only the contacts that actually resolved. The gate click bought the free look, not the reveal: never launch without an explicit pick + confirm. Close with one line on what it cost (one credit per contact revealed).
