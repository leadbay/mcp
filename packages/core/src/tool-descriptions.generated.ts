...
The result carries everything the user is waiting for: \`overall_progress:{done,total,done_ratio}\`, \`bulk_progress:{total_count,success_count,failure_count,quota_hit_count}\`, and (with \`include_contacts:true\`) \`leads[]\` each with contacts' \`email\` / \`phone\` / \`job_title\` / \`enrichment.done\`. Report it yourself in the SAME turn, without a reprompt and without deferring to a scheduled re-check: name which contacts now have emails / phones, the success / failure counts, and any \`quota_hit_count\` (if non-zero, say some contacts were skipped because the quota window was exhausted, and point to \`leadbay_account_status\` for the wait-or-top-up choice). If you stopped on a plateau (not \`all_done\`), say so plainly — report the resolved contacts and name the ones with no findable email, rather than implying the job fully finished. Then show refreshed quota via \`leadbay_account_status\` (see QUOTA above); do NOT print a credits-remaining line.
`;
