...
export const leadbay_bulk_enrich_status: string = `Check status + per-lead contacts for a bulk enrichment you previously launched via leadbay_enrich_titles. Returns progress per lead (done/total enrichable contacts), and overall progress. When \`include_contacts=true\` (opt-in), includes each contact's email/phone_number/job_title/enrichment.done.
...
WHEN NOT TO USE: for the qualification half — use leadbay_qualify_status with the \`notification_id\` the launch returned; or when you still want the legacy blocking behavior from leadbay_import_leads with \`wait_for_completion=true\`.
