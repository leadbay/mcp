...
\`not_imported\` rows with \`reason:"uncrawled"\` are **pending a background crawl**, NOT failures: the website is fine, Leadbay just hasn't indexed that domain yet and will add the lead asynchronously — surface them as pending. To SEE the leads Leadbay adds for them, re-check \`leadbay_pull_leads\` a bit later (\`leadbay_import_status\` only reports status/progress, not the added leads). A large \`uncrawled\` share on a fresh list is normal.
...
export const leadbay_import_status: string = `Retrieve the current **status/progress** of a lead import. Pass \`handle_id\` (only returned by an async \`leadbay_import_leads({wait_for_completion:false})\` run) — this resolves the stored result (leads + not_imported) when that async run has completed in this MCP instance. OR pass \`importIds[]\` (a completed import returns \`importIds\`; \`leadbay_import_and_qualify\` returns \`import_ids\`) to refresh the backend wizard rows' phase + record counts. Note: the \`importIds[]\` path returns status/progress only — it does NOT re-reconcile records or return refreshed leads/not_imported. This status call performs a single refresh pass and never polls in a loop.

WHEN TO USE: after an async import returns \`{status:'running', handle_id}\`, poll with that \`handle_id\`; OR to check whether a finished import is still processing. To actually SEE the leads Leadbay has since crawled and added for pending-crawl (\`uncrawled\`) rows, use \`leadbay_pull_leads\` a bit later — not this tool.
...
| Pending-crawl (\`uncrawled\`) rows     | "Leadbay is crawling them — check back shortly to see the ones it added" | leadbay_pull_leads (later) |
...
| Unmatched rows but websites present    | "Import anyway — Leadbay will crawl and match later"        | leadbay_import_leads; see the added leads via leadbay_pull_leads later |
