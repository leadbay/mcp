    ...
    if (!waitForCompletion) {
      ...
      if (claim.state === "in_flight") {
        throw client.makeError(
          "QUALIFY_LAUNCH_IN_FLIGHT",
          `An identical qualification was started ${claim.seconds_since}s ago and has not returned its job id yet`,
          "Nothing was launched twice. Call leadbay_bulk_qualify_leads again with the same arguments in a few seconds to receive the notification_id.",
          ""
        );
      }
