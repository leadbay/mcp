...
> = {
  ...
  execute: async (
    ...
    const alreadyQualified =
      qualifyClaim.state === "settled" ? qualifyClaim.record : undefined;
    if (alreadyQualified) {
      ctx?.logger?.info?.(
        `import_and_qualify: identical qualify ran ${alreadyQualified.seconds_since}s ago; re-running the fan-out to report current per-lead state (already-qualified leads are skipped inside it)`
      );
    }
    ...
    try {
      ...
      // The claim must not outlive a fan-out that did not run, or an identical
      // retry inside the window is told it already ran (product#4039).
      if (!alreadyQualified) abandonLaunch(qualifyFingerprint);
      throw err;
    }
