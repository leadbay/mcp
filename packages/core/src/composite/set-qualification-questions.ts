  ...
  inputSchema: {
    ...
    properties: {
      questions: {
        ...
        description:
          "Full replacement list of qualification questions (replaces ALL current questions). Mutually exclusive with add/remove. A question Leadbay can score has ALL of: the estimative marker — it starts \"Is the company likely to \" in English or \"L'entreprise est-elle susceptible de/d' \" in French (never \"Does the company ...\" or a bare \"L'entreprise est-elle <X> ?\", which the scorer cannot verify and marks no); ONE dimension, not two joined by AND; something estimable from the company's public material, never its budget or its internal plans; and enough bite to split companies roughly 30/70 — a question nearly everyone answers yes to (\"has a website\") adds no signal, so say that and propose a sharper one rather than writing it. Max 120 chars, in the user's language. It tests the lead as a BUYER, not as a category: would a company answering yes write a cheque to THIS user? \"Is the company likely to manufacture branded pharmaceuticals?\" for a seller of advertising scores the user's competitors and suppliers too; test the need the offering meets instead. Never name a specific company in a question. Never re-send an existing question with reworded text that means the same thing — a reword is a delete plus an add, it re-scores every lead in the pipeline against the org's quota, and it surfaces exactly the same companies.",
      },
      add: {
        type: "array",
        items: { type: "string", maxLength: 255 },
        description:
          "Questions to append to the current list (deduped). Mutually exclusive with `questions`. A question Leadbay can score has ALL of: the estimative marker — it starts \"Is the company likely to \" in English or \"L'entreprise est-elle susceptible de/d' \" in French (never \"Does the company ...\" or a bare \"L'entreprise est-elle <X> ?\", which the scorer cannot verify and marks no); ONE dimension, not two joined by AND; something estimable from the company's public material, never its budget or its internal plans; and enough bite to split companies roughly 30/70 — a question nearly everyone answers yes to (\"has a website\") adds no signal, so say that and propose a sharper one rather than writing it. Max 120 chars, in the user's language. It tests the lead as a BUYER, not as a category: would a company answering yes write a cheque to THIS user? \"Is the company likely to manufacture branded pharmaceuticals?\" for a seller of advertising scores the user's competitors and suppliers too; test the need the offering meets instead. Never name a specific company in a question. Read leadbay_get_qualification_questions first: skip anything an existing question already tests, and anything the lens already filters by sector, headcount or territory.",
      },
      remove: {
        type: "array",
        items: { type: "string" },
        description:
          "Exact question strings to remove from the current list. Mutually exclusive with `questions`. A removal requires confirm:true. Never remove a question in order to re-add a reworded version that means the same thing — even when the user asks you to clarify or improve the wording. That re-scores every lead, spends quota and surfaces exactly the same companies; offer instead to find out whether a question is testing the WRONG thing.",
      },
      add_anti_patterns: {
        type: "array",
        items: { type: "string" },
