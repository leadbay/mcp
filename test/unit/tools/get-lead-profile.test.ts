/**
 * Tests for leadbay_get_lead_profile.
...
import { registerGetLeadProfile } from "../../../src/tools/get-lead-profile.js";
...
function setup() {
  ...
  registerGetLeadProfile(t.api as any, client);
  return t;
}
...
describe("leadbay_get_lead_profile — success path", () => {
  it("returns all sections when every sub-request succeeds", async () => {
    ...
    const t = setup();
    const result: any = await executeTool(t, "leadbay_get_lead_profile", {
      leadId: LEAD,
      lensId: LENS,
    });
...
describe("leadbay_get_lead_profile — partial-result resilience", () => {
  ...
  it("lead fetch rejects (fatal) → throws", async () => {
    ...
    const t = setup();
    await expect(
      executeTool(t, "leadbay_get_lead_profile", { leadId: LEAD, lensId: LENS })
