...
// Install the node:https mock at module scope BEFORE any src imports.
...
describe("LeadbayClient.request — HTTP status → error code mapping", () => {
  const cases: Array<[number, string, object | string | undefined, string, string?]> = [
    [401, "AUTH_EXPIRED", { message: "expired" }, "re-authenticate"],
    [402, "QUOTA_EXCEEDED", { message: "no credits" }, "Purchase more credits"],
  ...
  it.each(cases)(
    "HTTP %i → error code %s",
    async (status, expectedCode, body, _expectedHint, expectedMessage) => {
...
describe("LeadbayClient.resolveDefaultLens", () => {
  ...
  it("re-fetches after 5-minute TTL expires", async () => {
    ...
    vi.setSystemTime(new Date("2026-04-20T00:06:00Z")); // 6 min later
    await expect(client.resolveDefaultLens()).resolves.toBe(2);
...
describe("LeadbayClient.resolveTasteProfile — partial-result resilience", () => {
  ...
  it("returns empty arrays when tags/questions reject but IBP succeeds", async () => {
    mockHttp([
      { method: "GET", path: "/1.5/users/me", status: 200, body: meBody },
      {
        method: "GET",
    ...
    const client = new LeadbayClient(BASE, "u.test-token");
    const tp = await client.resolveTasteProfile();
    expect(tp.idealBuyerProfile).toEqual({ summary: "ok" });
    expect(tp.purchaseIntentTags).toEqual([]);
    expect(tp.qualificationQuestions).toEqual([]);
  });
});
