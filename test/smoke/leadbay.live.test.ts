 ...
 *
 * Opt-in — set LEADBAY_TEST_TOKEN (and optionally LEADBAY_TEST_BASE_URL) to run.
 * Excluded from the default `npm test` via vitest.config.ts; run explicitly:
 *
 *     npm run test:smoke
 *
 * Governance (important before enabling in CI):
 *   - Use a DEDICATED test tenant — not a production Leadbay org
 *   - Use a LEAST-PRIVILEGED, READ-ONLY token
 *   - Smoke only hits read endpoints (/lenses, /users/me, taste profile)
 *   - No live login (email/password) — use token preconfigure to avoid
 *     session churn and audit-trail noise
 *   - No enrich / qualify / add-note calls from smoke — those cost credits
 */
