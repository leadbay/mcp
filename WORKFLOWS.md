...
|---|---|---|---|
| 1 | **Daily lead discovery** — "show me today's leads / fresh prospects / what's in my inbox". Every negative qualification answer stays visible beside the positive case; a lead is excluded only when the user's stated criteria make that answer a veto, and unavailable qualification is never presented as a pass. | `leadbay_daily_check_in` | "Show me today's leads" |
...
| 10 | **Field sales tour planning** — "I'm visiting Limoges in 4 days — give me 3 customers + 3 qualified + 3 new on one map". The itinerary reaches the next town over: `leadbay_tour_plan` adds the Discover leads within `radius_km` (default 20) of the named town, so West Sacramento is on a tour of Sacramento, and a user who names their own radius ("dans un rayon de 10km autour de Colmar") gets that one. Discover stops keep every negative qualification answer visible and apply only the user's explicit veto criteria. | `leadbay_plan_tour_in_city`, `leadbay_tour_plan` | "I'm visiting Jacksonville in 3 days — plan my visits" |
...
| 61 | **A launched job's result arrives in the same answer** — product#4178: "Qualifier ces 15 leads" came back still running, the agent ended its turn on that result, and the user got no verdicts. Every tool that starts background work (`leadbay_find_new_leads`, `leadbay_qualify_leads`, `leadbay_bulk_qualify_leads`, `leadbay_import_leads`, `leadbay_import_and_qualify`, `leadbay_enrich_titles`, `leadbay_enrich_contacts`, `leadbay_prepare_outreach`, `leadbay_new_lens`, `leadbay_extend_lens`, `leadbay_refine_prompt`, `leadbay_answer_clarification`) and every tool that checks it carries one rule, `gates/launched-job`: say what is running and how long it usually takes, check it in the same turn until the check tool reports it finished, then answer from the finished result. Ending the turn on "check on it in a minute" is the failure this row exists to stop. | `leadbay_qualify_leads`, `leadbay_lead_job_status` | "Qualifier ces 15 leads (écarte les agences immobilières / SCI, garde les vrais BTP/plaquistes)" |
...
success_criteria:
  ...
  - "showed every negative qualification answer beside the positive case, treated unavailable qualification as unknown rather than passed, and excluded a lead only when the user's stated criteria made that answer a veto"
...
success_criteria:
  ...
  - "preserved every negative qualification answer on Discover stops and applied only customer-stated veto criteria"
...
success_criteria:
  ...
  - "used qualification_summary.negative_answers without a second per-lead research call, excluding a lead only when a customer-stated criterion made the negative answer a veto"
