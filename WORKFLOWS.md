...
| 13 | **Lens management — list / switch audiences, read a lens's criteria** — "show me my lenses", "which audiences do I have", "switch to my Joinery lens", "what is my lens searching for" (every lens carries its criteria: sectors, locations and sizes by name — product#4176) | `leadbay_manage_lenses` | "Show me my lenses and switch to the Joinery one" |
...
| 32 | **Build an interactive artifact** — "build me a call sheet / interactive lead board with a campaign dropdown, notes, statuses, likes per lead" — the agent fetches headless view-models + usage guide via `leadbay_get_artifact_runtime`, then assembles a single-file HTML artifact whose `lb.field`/`lb.action` view-models POPULATE a dropdown from `leadbay_list_campaigns` and submit `leadbay_add_note` / `leadbay_set_prospecting_action` / `leadbay_add_leads_to_campaign` / `leadbay_like_lead` (carrying `_triggered_by` where required) — never `leadbay_report_outreach`, whose confirmation prompt a page cannot show; the artifact owns all rendering | `leadbay_get_artifact_runtime` *(no dedicated prompt)* | "Build me an interactive call sheet for these leads." |
...
| 58 | **A stated fit rule becomes a setting** — the core of product#4139: when the user says in chat what makes a lead good or bad ("écarte les sociétés liquidées", "je ne veux pas d'associations", "our best customers run their own maintenance crews", "the leads aren't relevant"), the agent must READ the org's settings, decide WHERE the rule belongs, decide whether a change is needed at all, and propose before writing. `leadbay_get_qualification_questions` now returns the ideal buyer profile and the targeting prompt alongside the questions, so coverage is checkable. A sector/size/territory rule goes to `leadbay_adjust_audience`; a qualitative orientation to `leadbay_refine_lead_targeting`; a named company to `leadbay_dislike_lead`, whose `reason` is saved as a note on the lead and whose response says the targeting did not change (product#4170), or to `leadbay_set_lead_status` with the reason written by `leadbay_add_note`, because a status stores none (product#4171); a kind of company the user rejects becomes an ideal-buyer-profile anti-pattern through `leadbay_set_qualification_questions`, unless the lead's `ai_score` is already below 0; CRM state and delivery requirements to neither. A question the scorer cannot act on comes back as `form_warnings`. Answering "c'est noté" with no tool call is the failure this row exists to stop. | `leadbay_get_qualification_questions`, `leadbay_set_qualification_questions`, `leadbay_refine_lead_targeting`, `leadbay_adjust_audience`, `leadbay_dislike_lead`, `leadbay_set_lead_status`, `leadbay_add_note` | "Écarte les sociétés liquidées, à risque manifeste, fabricants concurrents et comptes blacklistés" |
...
| 61 | **A launched job's result arrives in the same answer** — product#4178: "Qualifier ces 15 leads" came back still running, the agent ended its turn on that result, and the user got no verdicts. Every tool that starts background work (`leadbay_find_new_leads`, `leadbay_qualify_leads`, `leadbay_bulk_qualify_leads`, `leadbay_import_leads`, `leadbay_import_and_qualify`, `leadbay_enrich_titles`, `leadbay_enrich_contacts`, `leadbay_prepare_outreach`, `leadbay_new_lens`, `leadbay_extend_lens`, `leadbay_refine_lead_targeting`, `leadbay_answer_clarification`) and every tool that checks it carries one rule, `gates/launched-job`: say what is running and how long it usually takes, check it in the same turn until the check tool reports it finished, then answer from the finished result. Ending the turn on "check on it in a minute" is the failure this row exists to stop. | `leadbay_qualify_leads`, `leadbay_lead_job_status` | "Qualifier ces 15 leads (écarte les agences immobilières / SCI, garde les vrais BTP/plaquistes)" |
| 62 | **A broken artifact stops being invisible** — product#4081: an artifact built from `leadbay_get_artifact_runtime` runs in a chat-hosted page whose only channel out is the host bridge, so until now every runtime failure died there. The `@leadbay/components` runtime now reports six failure points itself, automatically, deduped and capped, through `leadbay_report_artifact_error`: a picker that loaded zero options, a button blocked by validation before any call, a call that timed out, failed, came back as an `{error:true}` envelope or a partial write, and text that would not parse. The MCP splits them the way it splits its own failures — the four exception kinds to Sentry under `source:"artifact"`, the three outcome kinds (where nothing threw, so Sentry would never see them) to the `mcp artifact event` PostHog stream. Every artifact-issued tool call also carries `_origin:"artifact"`, so artifact traffic is separable from agent traffic on the tool-call events that already exist. Only bounded enums and error codes travel — never a message, never user text. This is NOT `leadbay_report_friction`, which is consent-gated, carries the user's own words, and must never fire unprompted; a user who has opted out via `leadbay_set_telemetry` sends nothing at all. | `leadbay_report_artifact_error`, `leadbay_get_artifact_runtime` *(no dedicated prompt — the runtime calls it)* | "Build me an interactive call sheet for these leads." *(the campaign dropdown then loads empty)* |
| 63 | **Outreach session over any source** — "relance plan", "campagne de relance", "cold calling session", "prospection session", "let's contact leads", "who do I call today" — a working session over follow-ups, the active lens OR a campaign. Assesses reachability FIRST (a board of leads with no phone or email is a page the rep cannot work, so an unreachable page offers enrichment instead), builds the LEAD DESK via `leadbay_get_artifact_runtime`, then records each outcome as the rep dictates it — epilogue via `leadbay_report_outreach`, and `leadbay_set_lead_status` too when they report a commercial result, since those are different axes. For a campaign the user NAMES up front, `leadbay_work_campaign` stays the door. | `leadbay_outreach_session` | "Let's do a cold calling session on my follow-ups." |
| 64 | **Route session — a day in the field** — "plan my route", "tournée", "I'm driving to Lyon Thursday", "who can I visit in <city>", "prospection terrain" — the trip counterpart to the desk session. Picks the place (never a country — this workspace serves one), calls `leadbay_tour_plan`, repeats its in-city / within-radius split rather than presenting a lead from the next town over as being in the city, then builds the ROUTE PLANNER via `leadbay_get_artifact_runtime`: half map, half panel, clicking a marker opens that lead to set status and log the visit, with the pin repainting as the rep works. Leads with no coordinates are listed beside the map, never dropped. Visits are recorded one at a time via `leadbay_report_outreach`, plus `leadbay_set_lead_status` when the rep reports a commercial outcome too. | `leadbay_route_session` | "I'm driving to Lyon on Thursday — plan my route." |
| 65 | **Set or clear a prospecting action** — "untick Meeting planned, I clicked it by mistake", "set Bachelard to Still chasing", and every action button on a Leadbay board. The web app treats the four actions as a multi-select over the lead's `epilogue_today_statuses`: several can be on for one day and each toggles on its own. `leadbay_set_prospecting_action` does the same — `selected: true` adds one, `selected: false` removes it — and writes no note, so a mistaken tap is undone without a trace. When an outreach actually HAPPENED it is logged with `leadbay_report_outreach` instead, which writes the note and carries the proof. | `leadbay_set_prospecting_action` *(no dedicated prompt)* | "Untick Meeting planned on Solimac." |
...
forbidden_calls:
  - leadbay_report_outreach
success_criteria:
  - "called leadbay_refine_lead_targeting at least once with the user's instruction"
...
success_criteria:
  ...
  - "passed the place name (e.g. 'Indre-et-Loire') as a location, never as a sector or a leadbay_refine_lead_targeting call instruction"
...
success_criteria:
  ...
  - "rendered the delivery table and closed with the honest funnel line (matched/examined/delivered/stop reason)"
```
