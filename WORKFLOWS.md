...
| 46 | **Consent-gated problem report** — product#3943: the user explicitly asks for a Leadbay problem to be reported. `leadbay_report_friction` must fire with the user's own words, and the agent must show the confirmation back so the user knows what was sent. The underdeliver guard: an agent that treats the request as ordinary chatter and never reports is failing the user. | `leadbay_report_friction` | "Searching Wisconsin returns nothing — report this to the Leadbay team" |
...
success_criteria:
  ...
  - "told the user the report was sent (a visible confirmation), rather than reporting silently"
  - "did NOT claim to have sent anything it did not send"
render_checks:
  - "the final message confirms the report reached the Leadbay team"
```
