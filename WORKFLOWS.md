...
| 63 | **The layout reaches the agent that renders it** — Claude Code truncates every MCP tool description at 2,048 characters, so the rendering algorithm at the foot of a 16,947-character description was read by nobody: measured 2026-09-18, 75% of our description text sat past that cut and the rendering block survived it in 4 of the 21 tools that carried one. promptforge's `{{render}}` marker now lifts that block out of the description; every successful result of the tool carries `render: {recipe, guide}`, and `leadbay_render_guide` returns the full algorithm (score bar, columns, link rules, fallbacks) on demand. Migrated so far: `leadbay_pull_leads` (16,947 to 4,945 chars) and `leadbay_scan_portfolio_signals`. The ChatGPT surface serves the commerce-stripped block, so a top-up offer inside a layout cannot reach it through the guide. | `leadbay_render_guide`, `leadbay_pull_leads` | "Show me today's leads" |
