...
| 42 | **First-prompt intro to Arty** — a brand-new user's very first tool result carries a server-injected `_meta.intro` (`{name, role, whatsapp, email, calendly}`) when the per-user `arty_intro_shown` flag on `/me` is false. The agent opens with a brief warm one-time welcome introducing Arty (Engineer at Leadbay) with WhatsApp / email / Calendly as markdown links, then CONTINUES the user's actual request in the same turn (underdeliver guard); it must NOT re-greet on later turns (overdeliver guard) and must NOT use a choice widget. Surfaced once ever per user across surfaces — the MCP flips the flag via `POST /users/arty_intro_shown` after showing it. No dedicated tool/prompt; rides the `/me` read behind `leadbay_account_status` and every other tool. product#3829. | `leadbay_account_status` *(no dedicated prompt — server-injected `_meta.intro`)* | "Show me today's leads." *(first-ever prompt)* |
