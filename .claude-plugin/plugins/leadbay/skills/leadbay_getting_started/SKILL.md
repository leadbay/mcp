...
**FIRST, check your own tool set for `leadbay_enrich_titles` and
`leadbay_bulk_enrich_status`.** This walkthrough is exposed even on a read-only
connection (`LEADBAY_MCP_WRITE=0`), where those two are filtered out and the
reveal cannot happen. If either is missing: do **not** announce this gate, do
**not** fire its widget, and do **not** offer a button that dies on the click.
Say in one honest line that revealing contacts is switched off on this
connection and whoever set it up can enable it — then go straight to ENDING A.
Three gates that all worked beats four with a dead button.
...
Once confirmed, call `leadbay_enrich_titles` AGAIN with `leadIds` = **the same
one-element array you used in BEAT 1** (`leadIds: ["<the GATE 3 lead id>"]`),
the chosen `titles`, `confirm: true` and `email: true`. That's the real, paid
reveal.

**The key is `leadIds`, an array — never a singular `leadId`.** The tool reads
`leadIds` only; an unrecognised key is dropped, and with no `leadIds` it falls
back to the top of the wishlist. That would spend on several leads instead of
the one you drafted for.
...
> <https://calendly.com/zoe-leadbay/demo-leadbay>
   ...
   tour. **Do not append the stop line here** — the tour is over, there is no
   decision pending, and the offer is the last thing they see (see STOP).
...
## Where the stop line applies

The verbatim stop line below closes **every GATE hand-back** — you fired a gate
widget and are waiting for the click. That is the only place it belongs.

It does **NOT** apply to the three endings: the walkthrough is over there, so no
decision is pending. At ENDING B in particular the 1:1 offer is required to be
the last line, and appending the stop line under it would displace the offer —
the exact failure this prompt was already fixed for once. Ending A closes on the
setup link, ending C on the answer to what they actually asked.
